    ...
    <Dialog v-model:visible="visible" :style="{ width: '80%' }" :modal="true" maximizable>
        <template #header>
            ...
            <div class="flex gap-4">
                <Button type="submit" form="createTasksForm" icon="pi pi-save" label="Save" @click="handleSave" />

                <Button icon="pi pi-times" label="Cancel" @click="handleCancel" />
            </div>
        </template>
        <Form pt:root:id="createTasksForm" :resolver="resolver" @submit="handleSave"
            ...
            <FormField v-slot="$field" name="tasks" class="form-field">
                ...
                <TreeSelect v-model="selectedNodes" :options="selectOptions?.taskTree" filter filterMode="strict"
                    selectionMode="checkbox" display="chip" showClear />
                <Message v-if="$field?.invalid" size="small" variant="simple" severity="error">{{
                    $field.error?.message
                    }}</Message>
            </FormField>

            <DataTable :value="newTasks" dataKey="id" scrollable scrollHeight="flex" v-model:editing-rows="editingRows"
                edit-mode="row" v-model:expandedRowGroups="expandedRowGroups" expandableRowGroups
                rowGroupMode="subheader" groupRowsBy="task_category">
                ...
                <Column field="task_status" header="Task Status*" header-class="min-w-40">
                    <template #body="{ field, index }">
                        <Select v-model="newTasks[index][field]" :options="enums.TaskStatusEnum"
                            class="w-full"></Select>
                    </template>
                </Column>
                <Column field="start_year" header="Start Year*" header-class="min-w-48">
                    <template #body="{ index, field }">
                        <DatePicker @hide="onDataPickerHide" showIcon iconDisplay="input" showButtonBar
                            v-model="newTasks[index][field]" view="year" date-format="yy" :min-date="new Date()" />
                    </template>
                </Column>
                <Column field="expected_delivery_date" header="Expected Delivery Date*" header-class="min-w-56">
                    <template #body="{ index, field }">
                        <DatePicker @hide="onDataPickerHide" showIcon iconDisplay="input" showButtonBar
                            v-model="newTasks[index][field]" :min-date="new Date()" />
                    </template>
                </Column>
                ...
                <Column field="task_owner" header="Task Owner*" header-class="min-w-48">
                    <template #body="{ index, field }">
                        <Select v-model="newTasks[index][field]" :options="selectOptions.userOptions"
...
<script setup>
    ...
    async function getSelectOptions() {
        try {
            selectOptions = await Api.get('/tasks/select-options')
        } catch (error) {
            console.log(error)
        }
    }
