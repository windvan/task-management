<template>
  <Dialog v-model:visible="visible" modal maximizable @hide="emit('close')" :style="{ width: '80%' }">
    <template #header>
      <span class="text-xl font-bold">Gap Details</span>
      <div class="flex gap-8">
        <Button v-show="disabled" icon="pi pi-trash" rounded outlined @click="handleDeleteGap"></Button>
        <!-- <Button
          v-show="disabled"
          icon="pi pi-pencil"
          rounded
          ...
          @click="handleEditGap"
        ></Button> -->
        <Button v-show="!disabled" icon="pi pi-check" rounded outlined @click="handleSaveGap"></Button>
      </div>
    </template>

    <Image v-if="gap_id" :src="staticBaseUrl + gap?.snapshot_url" class="h-64 w-full border border-surface-200 rounded"
      preview pt:toolbar="bg-surface-600" />

    <FileUpload v-else ref="fileUploadRef" name="gap_snapshot" :fileLimit="1" mode="advanced" accept="image/*"
      :maxFileSize="1048576">
      <template #header="{ files, chooseCallback }">
        <Button icon="pi pi-plus" label="Choose" size="small" @click="chooseCallback"
          v-show="files.length === 0"></Button>
      </template>
      <template #empty>
    ...
    <div class="grid md:grid-cols-2 grid-cols-1 gap-8 border border-surface-200 rounded mt-4 p-2">
      <div class="flex flex-col grow gap-4">
        <div class="flex text-nowrap items-center">
          <label for="crop_name" class="mr-2">Crop Name</label>
          <InputText v-model="gap_detail['crop_name']" id="crop_name" fluid :disabled />
        </div>
        <div class="flex text-nowrap items-center">
          <label for="crop_name_cn" class="mr-2">Crop Name(CN)</label>
          <InputText v-model="gap_detail['crop_name_cn']" id="crop_name_cn" fluid :disabled />
        </div>
        <div class="flex text-nowrap items-center">
          <label for="control_target" class="mr-2">Control Target</label>
          <InputText v-model="gap_detail['control_target']" id="control_target" fluid :disabled />
        </div>
        <div class="flex text-nowrap items-center">
          <label for="control_target_cn" class="mr-2">Control Target(CN)</label>
          <InputText v-model="gap_detail['control_target_cn']" id="control_target_cn" fluid :disabled />
        </div>
        ...
        <div class="flex text-nowrap items-center">
          ...
          <InputText v-model="gap_detail['app_time_bbch']" id="app_time_bbch" fluid :disabled />
        </div>
      </div>
      <div class="flex flex-col grow gap-4">
        <div class="flex text-nowrap items-center">
          <label for="app_rate" class="mr-2">Application Rate</label>
          <InputText v-model="gap_detail['app_rate_min']" id="app_rate_min" fluid :disabled />
          <label for="app_rate" class="mx-2">-</label>
          <InputText v-model="gap_detail['app_rate_max']" id="app_rate_max" fluid :disabled />
          <label for="app_rate" class="ml-2">g a.i./ha</label>
        </div>
        <div class="flex text-nowrap items-center">
          <label for="water_valumn" class="mr-2">Water Valumn</label>
          <InputText v-model="gap_detail['water_valumn_min']" id="water_valumn_min" fluid :disabled />
          <label for="water_valumn" class="mx-2">-</label>
          <InputText v-model="gap_detail['water_valumn_max']" id="water_valumn_max" fluid :disabled />
          <label for="water_valumn" class="ml-2">L/ha</label>
        </div>
        <div class="flex text-nowrap items-center">
          <label for="app_method" class="mr-2">Application Method</label>
          <InputText v-model="gap_detail['app_method']" id="app_method" fluid :disabled />
        </div>
        <div class="flex text-nowrap items-center">
          <label for="app_number" class="mr-2">Application Number</label>
          <InputText v-model="gap_detail['app_number']" id="app_number" fluid :disabled />
        </div>
        <div class="flex text-nowrap items-center">
        ...
        <div class="flex text-nowrap items-center">
          ...
          <InputText v-model="gap_detail['app_time']" id="app_time" fluid :disabled />
        </div>
      </div>
      <div class="flex text-nowrap items-center col-span-2">
        <label for="additional_comments" class="mr-2">Additional Comments</label>
        <InputText v-model="gap_detail['additional_comments']" id="additional_comments" fluid :disabled />
      </div>
    </div>
  </Dialog>
</template>
