        ...
        <Form v-slot="$form" :resolver="resolver" :initialValues="initialFormData" ref="formRef"
            ...
            <FormField v-slot="$field" name="portfolio_contact" class="form-field">
                ...
                <AutoComplete inputId="portfolio_contact" optionLabel="name" :suggestions="portfolioContactSuggestion"
                    @complete="filterPortfolioContactSuggestion" forceSelection placeholder="Type to filter" fluid
                    dropdown />
                <Message v-if="$field?.invalid" size="small" variant="simple" severity="error">{{ $field.error?.message
                }}</Message>
            </FormField>
...
<script setup>
    ...
    async function filterPortfolioContactSuggestion(event) {
        portfolioContactSuggestion.value = await Api.get(`/users/search?query=${event.query}`);
    }
    ...
    const resolver = yupResolver(
        yup.object().shape({
            product: yup.mixed().label("Product").test('not-empty1', 'Product is required', (value) => {
                return value !== "" && value !== undefined && value !== null;
            }),
            registration_type: yup.string().required(),
            project_name: yup.string(),
            indication: yup.string().label("Indication").required(),
            portfolio_contact: yup.mixed().label('Portfolio Contact').test('not-empty', 'Portfolio Contact is required', (value) => {
                return value !== "" && value !== undefined && value !== null;
            }),
    ...
    //     return formRef.states.product?.value?.internal_name + "_" + formRef.states.registration_type?.value
    //     // return initialFormData.product?.internal_name + "_" + initialFormData.registration_type
    // })
