    ...
    <DataTable ref="projectTableRef" :value="projects" v-model:selection="selectedProject"
      ...
      <Column header="Action">
        <template #body="{ data, index }">
          <Button icon="pi pi-trash" @click="handleDeleteProject(data.id, index)" rounded outlined></Button>
        </template>
      </Column>
      ...
      <template #expansion="{ data }">
        <div class="mt-4 flex flex-col gap-4" @click="selectedProject = data">
          <div class="flex gap-4 items-center">
            ...
            <Button icon="pi pi-plus" severity="info" outlined size="small"
            ...
            <Column header="Action">
              <template #body="props">
                <Button icon=" pi pi-trash" rounded variant="outlined"
                  @click="handleDeleteProjectTask(data.id, props.data.id)"></Button>
              </template>
            </Column>
...
<script setup>
  ...
  function handleRefreshProject(project_id, newData) {
    const index = projects.value.findIndex((project) => project.id === project_id);
    if (index !== -1) {
  ...
  function handleShowProjectForm(mode, data) {
    ...
    if (mode === "new") {
      ...
      const product = { id: data.product_id, internal_name: data.product_internal_name, };
      const portfolio_contact = { id: data.portfolio_contact_id, name: data.portfolio_contact_name, };
  ...
  function handleDeleteProjectTask(project_id, task_id) {
    confirm.require({
      ...
      accept: async () => {
        ...
        //refresh tasks
        const projectIndex = projects.value.findIndex((project) => project.id === project_id);
        const taskIndex = projects.value[projectIndex].tasks.findIndex((task) => task.id === task_id);
  ...
  function handleShowTaskForm(mode, project, data) {
    if (mode === "new") {
      initialTaskFormData.value = { project_id: project.id, project_name: project.project_name };
    } else if (mode === "edit") {
