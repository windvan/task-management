    ...
    <Dialog
      ...
      <Form
        ...
        <FormField v-slot="$field" name="project_status" class="form-field">
          <label for="project_status" class="required-mark"
...
<script setup>
  ...
  function filterProductOptions(event) {
    if (event.query.trim()) {
      filteredProductOptions.value = selectOptions.productOptions.filter(
        (product) => {
  ...
  function handleNew() {
    ...
    initialFormData = {
      ...
      portfolio_contact: selectOptions?.userOptions.find(
        (user) => user.name === "Hank Gao"
      ),
    };
    showForm.value = true;
  }
    ...
    try {
      ...
      if (initialFormData.id) {
        ...
        response = await Api.patch(
          `/projects/${initialFormData.id}`,
          updatedFields
        );
        toast.add({
          severity: "success",
          summary: "Modify project successfully",
          life: 3000,
        });
      } else {
        // add new product
