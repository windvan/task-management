      ...
      <Column header="Action">
        <template #body="{data}">
          <Button icon=" pi pi-trash" rounded variant="outlined"
            @click="handleDeleteSample(data.id)"></Button>
        </template>
      </Column>
...
<script setup>
  ...
  async function handleDeleteSampleTask(sample_id, task_id) {
    ...
    const index = sample_list.value.findIndex((sample) => sample.id === sample_id);
    if (index !== -1) {
      sample_list.value[index].tasks = await Api.get(`samples/${sample_id}/tasks`);
    }
